...
if(analogRead(RIGHT_SENSOR)<=35 && analogRead(LEFT_SENSOR)<=35) //compare both sensor value to set the directionc
{
 ...
 {
  Serial.println("Bot will rotate towards right");
  motor1.run(FORWARD); // run motor1 clockwise
 ...
 {
  Serial.println("Bot will rotate towards left");
...
{
  //stop all the motors
  Serial.println("Bot will stop moving");
  motor1.run(RELEASE); 
  motor2.run(RELEASE);
  motor3.run(RELEASE);
  motor4.run(RELEASE);
